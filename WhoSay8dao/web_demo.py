...
def load_model_tokenizer(model_path):
    ...
    # 直接调用模型类
    model = MiniMindLM.from_pretrained(
        model_path,
        config=config,
...
MODEL_PATHS = {
    ...
    "WhoSay8dao(1.57M)": ["WhoSay8dao", "WhoSay8dao"]
}

selected_model = st.sidebar.selectbox('Models', list(MODEL_PATHS.keys()), index=8)  # 选择WhoSay8dao
# model_path = MODEL_PATHS[selected_model][0]
from pathlib import Path
model_path = str(Path(__file__).parent)  # 获取脚本所在目录（WhoSay8dao/）
